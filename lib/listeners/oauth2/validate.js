...
validate.token = (token, accessToken) => {
  ...
  return db_methods.clients.find(token)
...
validate.generateRefreshToken = ({ userID, clientID, scope }) => {
  ...
  return db_methods.refreshTokens.save(refresh_token)
  .then(() => refreshToken);
};
...
validate.generateToken = ({ userID, clientID, scope }) => {
  ...
  return db_methods.accessTokens.save(access_token)
  .then(() => token);
};
